...
def html(cenas):
    ...
    for i,c in enumerate(cenas):
        if c['tipo']=='dor':
            ...
            partes.append(f'<div class="cena dor" id="c{i}"><div class="topo">{LOGO}</div><div class="t h">Você conhece essa <em class="esc">semana.</em></div><ul class="lista">{its}</ul><div class="rod">e o chefe ainda pergunta "cadê o relatório?"</div></div>')
            extra.append(f'[0.4,1.9,3.6].forEach((s,k)=>setTimeout(()=>document.getElementById("d"+k).classList.add("on"),(T{i}+s)*1000));')
        elif c['tipo']=='custo':
            partes.append(f'<div class="cena custo" id="c{i}"><div class="topo">{LOGO}</div><div class="t h">Quanto isso custa?</div><div class="num"><span id="cnum">0</span> h<small>por ano montando do zero</small></div><div class="barra"><i id="cbar"></i></div><div class="leg">6 horas por semana × 50 semanas. Tempo que não volta.</div></div>')
            extra.append(f'setTimeout(()=>{{document.getElementById("cbar").style.width="100%"; let n=0; const iv=setInterval(()=>{{n+=6; if(n>=300){{n=300;clearInterval(iv);}} document.getElementById("cnum").textContent=n;}},60);}},(T{i}+0.5)*1000);')
        elif c['tipo']=='fluxo':
            ps=[('Preencher','a planilha já vem pronta'),('Perguntar','cola os números no prompt'),('Entregar','a IA escreve, você revisa')]
            inner=''.join(f'<div class="p" id="f{k}"><div class="n">{k+1}</div><div><b>{a}</b><span>{b}</span></div></div>'+('<div class="seta">▼</div>' if k<2 else '') for k,(a,b) in enumerate(ps))
...
out=ROOT/'trabalho-venda'; out.mkdir(exist_ok=True); wavs=[]
for i,c in enumerate(CENAS):
...
final=ROOT/'essencial-venda-9x16.mp4'
...
(ROOT/'essencial-venda-9x16.srt').write_text('\n'.join(srt)); print(f'{total:.1f}s', final)
