import openpyxl,pathlib,sys,datetime,re
...
for pasta,nome,kit in KITS:
    ...
    for f in files:
        wb=openpyxl.load_workbook(f); nf=ne=nv=nsb=nsolta=nx=ncf=ncfx=nlit=0
        ...
        out.append(f"| `{f.name}` | {' · '.join(wb.sheetnames)} | {nf} | {ne} | {nv} | {ncf} | {nsb} | {nsolta} | {nx} | {ncfx} | {nlit} |")
