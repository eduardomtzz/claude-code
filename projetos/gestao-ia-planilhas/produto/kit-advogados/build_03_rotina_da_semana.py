#!/usr/bin/env python3
"""Planilha 3 do Kit de Gestão para Advogados: Rotina da semana do escritório. Gera 03-rotina-da-semana.xlsx"""
from ssg import *
import dados, random
from datetime import date
NI=24; R0=5; RI=R0+NI-1; W=52; C0=5; CW=C0+W-1; NRESP=10   # itens 5..28; semanas nas colunas E..BD
LW=L(CW); RT=RI+2  # linha dos totais: 30 feitas, 31 registradas, 32 planejadas, 33 %, 34 minutos
wb=Workbook()
# ---------- Config ----------
...
titulo(ro,"Rotina da semana","Linhas: as rotinas fixas de segunda (prazos) e de sexta (caixa e painel). Colunas: uma por semana. Marque Sim ou Não em cada semana; os totais e o Painel são calculados.",merge_to="D")
...
itens=[("Segunda","Abrir a Agenda de prazos: tratar os atrasados. Prazo processual, conferir na fonte oficial e levar ao responsável, sem mudar a data. Reagendar só o que é tarefa administrativa.",4,"Marina Ferraz"),
       ...
       ("Sexta","Lançar as entradas e saídas da semana no caixa",5,"Marina Ferraz"),
       ("Sexta","Marcar as parcelas recebidas e as atrasadas",3,"Marina Ferraz"),
       ("Sexta","Registrar as horas da semana por caso",4,"Rafael Lima"),
       ("Sexta","Enviar a cobrança educada das parcelas atrasadas",3,"Rafael Lima"),
       ("Sexta","Olhar o painel do escritório e anotar até 3 decisões",3,"Marina Ferraz")]
...
widths(p,(12,52,10,16,10,10,11,26,3,26,12)); p.freeze_panes="A4"; p.sheet_view.showGridLines=False
# ---------- Como usar ----------
como_usar(wb,"Rotina da semana do escritório",[
 ("O que esta planilha faz","Fixa a rotina de gestão em dois momentos curtos: segunda (prazos) e sexta (caixa e painel). Você marca, semana a semana, o que foi feito; o Painel mostra a aderência das últimas 4 semanas, por dia e por rotina."),
 ("Passo 1","Em Config, troque a data de referência por =HOJE() (no exemplo ela está congelada em 14/09/2026), a segunda-feira da semana 1 (sugestão: a primeira do ano) e as pessoas do escritório."),
