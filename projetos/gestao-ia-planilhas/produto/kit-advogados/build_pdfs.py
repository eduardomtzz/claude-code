#!/usr/bin/env python3
"""Gera os PDFs do Kit de Gestão para Advogados (manual, prompts, bônus, checklists) a partir do Markdown, com a marca,
...
RODAPE='Seu Sócio Gestor · Kit de Gestão para Advogados · página <span class="pageNumber"></span> de <span class="totalPages"></span>'
...
/* Manual: cada planilha (título, texto, recorte com legenda e "detalhe que importa") fica junta na mesma página;
   recortes em largura total, com no máximo 70 mm de altura. */
.planilha{{page-break-inside:avoid;break-inside:avoid;margin-bottom:6pt}}
.planilha p{{margin:0 0 4pt;line-height:1.36}} .planilha h3{{margin-top:8pt}}
figure.fig{{margin:4pt 0 6pt;text-align:center;page-break-inside:avoid;break-inside:avoid}}
figure.fig img{{max-height:52mm;width:auto;max-width:100%;margin:0}}
...
.check2 h1+p{{font-family:Figtree,system-ui,sans-serif;font-size:11pt;color:#5A4A78;margin:0 0 14pt}}
...
.cols ul.check{{margin:0 0 10pt}} .cols ul.check li{{margin-bottom:2.5pt;break-inside:avoid}}
.cols .rodape{{font-family:'IBM Plex Mono';font-size:8.5pt;color:#7A5AA8;margin-top:6pt}}
"""
TITULOS={'21-biblioteca-de-prompts-do-escritorio':'Biblioteca de prompts do escritório',
 '22-mensagens-de-cobranca-e-confirmacao':'Mensagens de cobrança e confirmação',
 '23-guia-lgpd-escritorio-pequeno':'Guia LGPD para o escritório pequeno',
 '24-roteiro-reuniao-com-o-contador':'Roteiro da reunião com o contador',
 '25-checklists':'Checklists do escritório',
...
RECORTES={'01':(0,0,1950,700),'02':(0,0,1950,640),'03':(0,0,1890,630),'04':(0,0,1950,720),'05':(0,0,1850,760),
 '06':(0,0,1870,700),'07':(0,0,1925,700),'08':(0,0,1380,700),'09':(0,0,1850,760),'10':(0,0,1950,700),'11':(0,0,1950,480),
 '12':(0,0,1950,720),'13':(0,0,2080,700),'14':(0,0,1950,810),'15':(0,0,1950,775),'16':(0,0,1910,610),'17':(0,0,2090,840),
 '18':(0,0,1815,645),'19':(0,0,1890,530),'20-painel':(0,0,2000,600),'20':(0,0,1890,720)}
def recortes():
...
def build(md_path, pdf_name, titulo, sub, capa=True, classe=''):
    ...
    capa_html=f'<div class="capa"><div>{logo}</div><div><div class="t">{titulo}</div><div class="s">{sub}</div></div><div class="m">Kit de Gestão para Advogados · versão 1.0 · setembro de 2026 · seusociogestor.com.br</div></div>' if capa else ''
    title=f'{TITULOS[pdf_name]} · Kit de Gestão para Advogados'
...
if __name__=='__main__':
    ...
    jobs=[build('26-manual-de-implantacao.md','26-manual-de-implantacao','Manual de implantação: <em>quatro semanas, cinco núcleos</em>','As 20 planilhas, a ordem para começar, a rotina de segunda e de sexta, e os erros comuns. Leia uma vez; depois é só rotina.',classe='manual'),
      build('21-biblioteca-de-prompts-do-escritorio.md','21-biblioteca-de-prompts-do-escritorio','40 prompts <em>do escritório</em>','Explicar o mês, cobrar com educação, preparar a reunião com o contador, revisar a proposta pela margem. Nenhum produz peça ou parecer.',classe='prompts'),
      build('22-mensagens-de-cobranca-e-confirmacao.md','22-mensagens-de-cobranca-e-confirmacao','15 mensagens de <em>cobrança e confirmação</em>','E-mail e WhatsApp, tom educado, campos entre colchetes, na régua da planilha 14. Bônus do kit.',classe='msgs'),
      build('23-guia-lgpd-escritorio-pequeno.md','23-guia-lgpd-escritorio-pequeno','Guia LGPD para o <em>escritório pequeno</em>','O que guardar, onde, por quanto tempo, quem acessa, e o que nunca colar em IA pública. Bônus do kit.',classe='lgpd'),
      build('24-roteiro-reuniao-com-o-contador.md','24-roteiro-reuniao-com-o-contador','Roteiro da reunião mensal <em>com o contador</em>','Pauta de 30 minutos, o que levar do kit, o que perguntar, o que anotar. Bônus do kit.',classe='contador'),
    ...
    if '--pdf' in sys.argv:
        ...
        for nome,rot in (('21-biblioteca-de-prompts-do-escritorio','prompt'),('22-mensagens-de-cobranca-e-confirmacao','mensagem')):
