...
jobs=[build('26-manual-de-implantacao.md','26-manual-de-implantacao','Manual de implantação: <em>quatro semanas, cinco núcleos</em>','As 20 planilhas, a ordem para começar, a rotina de segunda e de sexta, e os erros comuns. Leia uma vez; depois é só rotina.',classe='manual'),
      build('21-biblioteca-de-prompts-do-escritorio.md','21-biblioteca-de-prompts-do-escritorio','40 prompts <em>do escritório</em>','Explicar o mês, cobrar com educação, preparar a reunião com o contador, revisar a proposta pela margem. Nenhum produz peça ou parecer.'),
      build('22-mensagens-de-cobranca-e-confirmacao.md','22-mensagens-de-cobranca-e-confirmacao','15 mensagens de <em>cobrança e confirmação</em>','E-mail e WhatsApp, tom educado, campos entre colchetes. Bônus do kit.'),
      build('23-guia-lgpd-escritorio-pequeno.md','23-guia-lgpd-escritorio-pequeno','Guia LGPD para o <em>escritório pequeno</em>','O que guardar, onde, por quanto tempo, quem acessa, e o que nunca colar em IA pública. Bônus do kit.'),
      build('24-roteiro-reuniao-com-o-contador.md','24-roteiro-reuniao-com-o-contador','Roteiro da reunião mensal <em>com o contador</em>','Pauta de 30 minutos, o que levar do kit, o que perguntar, o que anotar. Bônus do kit.'),
      build('25-checklists.md','25-checklists','Checklists','',capa=False,classe='check2')]
print('\n'.join(str(j) for j in jobs))
if '--pdf' in sys.argv:
    dest=sys.argv[sys.argv.index('--pdf')+1]; imprimir(jobs, dest)
    # txt dos prompts (Markdown inteiro) para copiar no celular
    pathlib.Path(dest,'21-biblioteca-de-prompts-do-escritorio.txt').write_text(pathlib.Path('21-biblioteca-de-prompts-do-escritorio.md').read_text())
