"""Varredura de entradas: cada célula de entrada preenchida do exemplo recebe vazio, texto e -1;
recalcula no LibreOffice pt-BR e compara TODAS as fórmulas com o exemplo. Sinaliza:
ERR  erro de fórmula;  NUM  número que mudou de valor sem aviso;  CLS  rótulo que trocou por outro
rótulo (sem palavra de aviso). Uso: python3 fuzz.py <arquivo relativo a produto/> <saida.json>"""
...
AVISO=re.compile(r'falta|Falta|inválid|incomplet|texto|sem base|Faltam|Corrija|Complete|Preencha|Sem recomendação|cadastro|Sem base|Estime|Margens|margens|Regra de risco|volume|Não há|preço inválido|Preço inválido|dados',re.I)
...
for k,cs in cols.items():
    esc=[cs[0]] if len(cs)<3 else [cs[0],cs[len(cs)//2],cs[-1]]
    for c in esc:
        alvos.append((c.parent.title,c.coordinate,c.value))
casos=[]
...
for i,(sh,co,v,modo) in enumerate(casos):
    wb=openpyxl.load_workbook(W/'calc'/nomes[i],data_only=True); r={'entrada':f'{sh}!{co}','modo':modo,'ERR':[],'NUM':[],'CLS':[]}
    for s2,c2 in fcells:
...
print(arq,len(casos),'casos;',sum(bool(r['ERR']) for r in res),'com erro;',sum(bool(r['NUM']) for r in res),'com número mudado;',sum(bool(r['CLS']) for r in res),'com rótulo mudado')
