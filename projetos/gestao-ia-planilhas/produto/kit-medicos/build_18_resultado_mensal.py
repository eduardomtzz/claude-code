#!/usr/bin/env python3
"""Planilha 18 do Kit de Gestão para Médicos: Resultado Mensal (DRE simplificada da clínica). Gera 18-resultado-mensal.xlsx
Receita por categoria do caixa (as seis categorias de entrada da 09), custos fixos (dados.CUSTOS_FIXOS), despesas variáveis
(materiais, taxas de cartão, repasse à parceira, manutenção), pró-labore fixo, impostos provisionados (11 %), resultado e margem, mês a mês.
Exemplo: janeiro a agosto de 2026 (agosto = último mês fechado), valores iguais ao Painel mensal da 09 por categoria."""
from ssg import *
import dados
...
cfg["C7"]="Use a alíquota efetiva que o contador informar (Simples anexo III ou V, ou lucro presumido). A provisão sai todo mês, mesmo que o imposto seja pago depois. No exemplo, 11 %, a mesma das planilhas 05 a 10."; nota(cfg["C7"])
...
titulo(rs,"Resultado mensal da clínica","Preencha as linhas amarelas de cada mês com os totais por categoria do Painel do caixa (planilha 09). Totais, impostos provisionados, resultado e margem são calculados. Meses futuros ficam em branco.",merge_to="O")
...
def linhas_entrada(r0,nomes,valor):
    for j,nome in enumerate(nomes):
        ...
        for m in range(12):
            ...
            if m<NMESES: cell.value=round(valor(m+1,nome),2)
...
secao(R_VA0-1,"Despesas variáveis (materiais, taxas de cartão, repasse, manutenção)")
linhas_entrada(R_VA0,VARIAVEIS,lambda m,n: DRE[m]["variaveis"][n])
linha_total(R_VAT,"Total de despesas variáveis",lambda col: f'=IF({PREENCH(col)},"",SUM({col}{R_VA0}:{col}{R_VAT-1}))')
...
widths(rs,[50]+[11]*12+[14,16]); rs.freeze_panes="B5"; rs.sheet_view.showGridLines=False
...
proteger(wb); salvar(wb,"18-resultado-mensal.xlsx","Resultado Mensal · Kit de Gestão para Médicos")
