...
import dados
NPROC=12; NPAG=6; T0=6; TN=T0+NPROC-1
wb=Workbook()
# ---------- Config ----------
...
campos=[("Nome da clínica",f"{dados.CLINICA} (exemplo fictício)",None),("Mês do volume (mês fechado)","Agosto de 2026",None),("Data de referência","=TODAY()",DATA),
 ("Custo da hora de atendimento (R$)",dados.CUSTO_HORA,BRL),("Impostos e taxas sobre o que entra (%)",dados.ALIQ,PCT),("Margem mínima sobre o preço (%)",dados.MARGEM,PCT),("Margem alvo sobre o preço (%)",dados.MARGEM_ALVO,PCT),
...
cfg["C7"]="Copie do Painel da planilha 05 (\"Custo da hora de atendimento\"). No exemplo, R$ 200,00."; cfg["C8"]="A mesma alíquota das planilhas 05, 06 e 07 (exemplo: 11 %)."
...
heads=["Procedimento","Minutos","Gera retorno?","Material (R$)","Custo cheio (R$)","Preço mínimo (R$)","Preço alvo (R$)"]+PAG_H+["Situação do particular","Tabelas abaixo do custo cheio","Realizados no mês","Produção no mês (R$)","Valor médio praticado (R$)","Valor médio contra o mínimo","Referência de mercado (opcional)"]
...
for r in range(T0,TN+1):
    ...
    t.cell(row=r,column=15,value=f'=IF(A{r}="","",SUMPRODUCT((I{r}:M{r}<>"")*(I{r}:M{r}>0)*(I{r}:M{r}<E{r})))'); calc(t.cell(row=r,column=15),"0")
...
t.conditional_formatting.add(f"I{T0}:M{TN}", FormulaRule(formula=[f'AND(ISNUMBER(I{T0}),I{T0}>0,I{T0}<$E{T0})'], fill=fill(VERM), font=F(color=VERM_T,size=10)))
t.conditional_formatting.add(f"I{T0}:M{TN}", FormulaRule(formula=[f'AND(ISNUMBER(I{T0}),I{T0}>=$E{T0},I{T0}<$F{T0})'], fill=fill(AMARELO)))
...
kpi(t,4,5,"Tabelas de convênio abaixo do custo",f"=SUM({KO})",VERM,VERM_T,fmt="0")
...
NT=TN+2
notas=["Custo cheio, preço mínimo e preço alvo: a mesma conta da planilha 06 (tempo com retorno × custo-hora + material; ÷ (1 − impostos − margem)). Tabela de convênio em vermelho: abaixo do custo cheio; amarela: cobre o custo, mas não a margem mínima.",
...
for i in range(NPROC):
    ...
    t.cell(row=r,column=2,value=f'=IF($A{s}="","",B{s}+IF(C{s}="Sim",{NRET}*{DRET},0))'); calc(t.cell(row=r,column=2),"0.0")
    for j in range(NPAG):
        c=3+j; src=f"{L(8+j)}{s}"
...
widths(t,(24,8,9,10,11,11,11,11,11,11,11,9,9,18,11,10,13,12,11,22)); t.freeze_panes="B6"; t.sheet_view.showGridLines=False
# ---------- exemplo ----------
...
como_usar(wb,"Tabela de preços e referência por procedimento",[
 ...
 ("Passo 2","Em Tabela, uma linha por procedimento: minutos, se gera retorno, material e o valor de cada pagador. A tabela de preços da agenda (01, Config) deve ser esta."),
