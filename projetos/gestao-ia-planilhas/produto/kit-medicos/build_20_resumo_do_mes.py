#!/usr/bin/env python3
"""Planilha 20 do Kit de Gestão para Médicos: Resumo do Mês para a IA e para o contador. Gera 20-resumo-do-mes.xlsx
...
Exemplo: agosto × julho de 2026 (último mês fechado), números do Histórico da 17 e da 18 (fonte única: dados.py)."""
from ssg import *
import dados
NI=12; R0=5; RN=R0+NI-1
...
ex=[("Entrou no mês (recebimentos)","R$",0,46000,"Sim",H7["entrou"],H8["entrou"],"09 · Caixa (Painel) ou 17 · Painel (Histórico)"),
    ...
    ("Taxa de falta","%",1,6,"Não",r1(H7["taxa_falta"]*100),r1(H8["taxa_falta"]*100),"02 · Faltas e retornos ou 17 · Histórico"),
    ("Horas atendidas no mês","h",1,140,"Sim",r1(H7["atendidas"]),r1(H8["atendidas"]),"01 · Agenda (Painel) ou 17 · Histórico"),
    ("Convênio a receber (lotes enviados)","R$",0,None,"Sim",H7["convenio"],H8["convenio"],"13 · Convênios (fim do mês) ou 17 · Histórico"),
...
rs["A1"]="Resumo do mês, pronto para a IA e para o contador"; rs["A1"].font=F(bold=True,size=16,color=UVA); rs.merge_cells("A1:D1")
...
rs.cell(row=T0+1,column=1,value=f'=A4&CHAR(10)&CHAR(10)&"Indicadores do mês:"&CHAR(10)&{partes}&CHAR(10)&"Destaques:"&CHAR(10)&{destaques}&IF(A6="","",CHAR(10)&"Observações da clínica: "&A6)')
...
rs.row_dimensions[T0+1].height=380
rs.cell(row=T0+3,column=1,value="Os textos são gerados por fórmula. Revise antes de enviar: a IA e o contador precisam do porquê dos números, e isso só a clínica sabe. Nenhum dado de paciente entra aqui: só totais. Variação de indicadores em % em pontos percentuais (p.p.).").font=F(size=9,color=LILAS)
...
wb._sheets=[wb["Painel"],wb["Resumo"],wb["Config"],wb["Indicadores"]]
...
como_usar(wb,"Resumo do Mês",[
 ...
 ("Passo 1","Em Config, preencha a clínica, o ano e escolha o mês do resumo (o último mês fechado; no exemplo, agosto de 2026)."),
 ("Passo 2","Em Indicadores, troque os exemplos pelos seus (de cima para baixo, sem pular linha): nome, unidade, casas decimais, meta e se maior é melhor. Depois, o valor do mês anterior e do mês atual, copiados do Painel da clínica (17, aba Histórico) e do Resultado mensal (18). Em %, digite 5,1 e não 0,051."),
 ...
 ("Cuidado com dados","O bloco só tem totais da clínica. Não cole nome de paciente, contato ou qualquer informação clínica na IA: veja o guia LGPD do kit."),
])
proteger(wb); salvar(wb,"20-resumo-do-mes.xlsx","Resumo do Mês · Kit de Gestão para Médicos")
