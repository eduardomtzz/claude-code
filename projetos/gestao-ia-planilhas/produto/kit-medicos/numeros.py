...
w(tab(["Pagador (01, setembro)","Realizados","Horas","Faltas","Taxa de falta","Produção","Valor médio","% da produção"],[(a,b,n(h,1),c,pct(d,0) if d not in (None,"") else "—",brl(e),brl(f),pct(g,0)) for a,b,h,c,d,e,f,g in [row(p01,r,(7,8,9,10,11,12,13,14)) for r in range(44,48)]]))
...
w(f"- Taxas do exemplo (Config): Pix 0 %, débito 1,5 % (D+1), crédito à vista 3,2 % (D+30), crédito parcelado 3,9 % (parcelas de 30 em 30 dias); sem antecipação. Vendas na aba: {len([r for r in dados.AGENDA_TODA if r['situacao']=='Realizado' and r['pagador']=='Particular' and r['forma'] in ('Pix','Cartão de débito','Cartão de crédito') and r['data']+datetime.timedelta(days=dados.DIAS_CREDITO.get(r['forma'],0)+30*((r['parcelas'] or 1)-1))>=dados.INICIO_AGENDA])} (toda venda com alguma parcela caindo a partir de 01/06: as da Agenda, de 01/06 a 11/09, mais as de abril e maio com parcela ainda a cair). Taxas de julho (saída do caixa em 31/07): {brl(T[7]['por_cat'].get('Taxas de cartão',0),2)}; de agosto (31/08): {brl(T[8]['por_cat'].get('Taxas de cartão',0),2)}.\n")
