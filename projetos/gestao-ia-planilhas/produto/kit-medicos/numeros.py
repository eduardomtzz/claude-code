...
w("- **Hoje (data de referência, Config = HOJE())**: segunda-feira **14/09/2026**. **Sexta do painel**: **11/09/2026**. Nenhum lançamento pago (caixa, parcela, lote de convênio) tem data depois de 11/09/2026.")
...
w(f"- **Alíquota de impostos** (05 a 10, 18): **{dados.ALIQ*100:.0f} %** (alíquota efetiva combinada com o contador; Simples anexo III/V simplificado). **Margem mínima** 30 %, **margem alvo** 45 %, **retornos por consulta** 0,4 × 20 min (embutidos no custo da consulta), **custo do dinheiro** (07) 1,5 % ao mês, **repasse** da médica parceira 50 % da produção.")
...
w("- **Datas relativas**: agenda futura (01/02), vencimentos em aberto (14) e orçamentos abertos (15) são `=HOJE()+n`; ao abrir em outro dia, esses números deslizam. Histórico (atendimentos realizados, lotes, parcelas pagas, caixa) é fixo.")
...
w(f"- 06 Resumo: procedimentos com particular abaixo do mínimo: {s06['E24'].value}; tabelas de convênio que não cobrem o custo cheio: {s06['E25'].value}; maior prejuízo por atendimento em convênio: {brl(s06['E26'].value,2)}. Simulação de exemplo (consulta estendida de 45 min a R$ 480): custo {brl(s06['B36'].value,2)}, mínimo {brl(s06['B37'].value,2)}, alvo {brl(s06['B38'].value,2)}, margem {pct(s06['B39'].value)}.")
w(f"- 08 Tabela (volume de agosto, Painel da 01): produção do mês {brl(t08['G5'].value)}; valor médio por hora {brl(t08['I5'].value,2)} ({pct(t08['K5'].value,0)} contra a hora mínima); particular abaixo do mínimo: {t08['C5'].value}; tabelas de convênio abaixo do custo: {t08['E5'].value}.")
w(tab(["Procedimento (08)","Realizados em agosto","Produção","Valor médio praticado","Contra o mínimo"],[(a,b,brl(c),brl(d,2),pct(e,0) if e not in (None,"") else "—") for a,b,c,d,e in [row(t08,r,(1,16,17,18,19)) for r in range(6,13)]]))
w(tab(["Valor por hora por pagador (08)","Minutos c/ retorno","Particular","Saúde Total","MediPlan","Vida Care","Hora mínima","Hora alvo"],[(a,n(b,1),brl(c),brl(d) if d else "—",brl(e) if e else "—",brl(f) if f else "—",brl(i),brl(j)) for a,b,c,d,e,f,g,h,i,j in [row(t08,r,list(range(1,11))) for r in range(25,32)]]))
...
w(tab(["11 · Parceira (mês / ano até setembro)","Produção no mês","Repasse devido no mês","Produção no ano","Repasse no ano","Pago no ano","A pagar","Fica com a clínica","Custo da estrutura (horas × R$ 71,43)","Margem da parceria"],[(a,brl(b),brl(c),brl(d),brl(e),brl(f),brl(g),brl(h),brl(i),brl(j)) for a,b,c,d,e,f,g,h,i,j in [row(p11,9,list(range(1,11)))]]))
w(tab(["Repasse (11), mês a mês","Produção","Horas atendidas","Repasse devido (50 %)","Pago em","Valor pago","Fica com a clínica","Margem da parceria"],[(a,brl(d),n(e,1),brl(g),dt(h),brl(i) if i else "—",brl(k),brl(m)) for a,b,c,d,e,f,g,h,i,j,k,l,m in [row(rp,r,list(range(1,14))) for r in range(5,14)]]))
...
lin=[(6,"Particular à vista"),(7,"Particular a prazo"),(8,"Convênio · Saúde Total"),(9,"Convênio · MediPlan"),(10,"Convênio · Vida Care"),(11,"Outras entradas"),(12,"**Receita total**"),(22,"Custos fixos (8 linhas)"),(24,"Materiais e insumos"),(25,"Taxas de cartão"),(26,"Repasse à médica parceira"),(27,"Manutenção de equipamentos"),(28,"**Despesas variáveis**"),(32,"Pró-labore fixo (Carolina 9.000 + Paulo 9.000)"),(33,"Impostos provisionados (11 % da receita)"),(34,"**Total de saídas**"),(35,"**Resultado do mês**")]
w(tab(["Linha da DRE","Julho","Agosto","Jan–ago (total)","Média jan–ago"],[(nm,brl(r18.cell(row=r,column=8).value),brl(r18.cell(row=r,column=9).value),brl(r18.cell(row=r,column=14).value),brl(r18.cell(row=r,column=15).value)) for r,nm in lin]+[("**Margem (resultado ÷ receita)**",f"**{pct(r18['H36'].value)}**",f"**{pct(r18['I36'].value)}**",pct(r18['N36'].value),pct(r18['O36'].value))]))
w(f"- Agosto: receita **{brl(p18['A5'].value)}**, saídas **{brl(p18['C5'].value)}**, resultado **{brl(p18['E5'].value)}**, margem **{pct(p18['G5'].value)}** (julho: {pct(r18['H36'].value)}). Previsto de agosto: receita 46.000, custos fixos 10.000, variáveis 6.000, pró-labore 18.000.")
...
w("- A DRE não inclui retiradas extras, distribuição de lucro nem despesas pessoais dos sócios (ficam na 11); por isso \"Saiu no mês\" do caixa é diferente do \"Total de saídas\" da DRE (impostos são provisão de 11 % da receita, e não a guia paga).\n")
...
w(f"- Guias na aba Guias (jul–set): {sum(1 for g in dados.GUIAS if g['data']>=dados.INICIO_AGENDA)} (uma por atendimento de convênio realizado); glosadas: {sum(1 for g in dados.GUIAS if g['data']>=dados.INICIO_AGENDA and g['glosada'])}. Lotes de out/2025 a jun/2026 vêm dos registros anteriores à agenda (sem detalhe por guia).")
...
w(tab(["Meta de caixa do trimestre (12)","Alvo","Atual","Progresso","Prazo","Situação"],[(a,brl(b),brl(c),pct(d,0),dt(f),h) for a,b,c,d,e,f,g,h in [row(p12,r,(1,2,3,4,5,6,7,8)) for r in (37,38,39)]]))
...
# ---------- 12. histórico 17 ----------
h17=W["17"]["Histórico"]
w("## 12. Histórico mensal (17 · Histórico; jan–ago = Painel mensal da 09, agenda desde julho, convênios/parcelas no fim de cada mês; setembro = Dados)\n")
...
w("- Agenda: a planilha 01 começou em julho (antes fica em branco). Orçamentos: o funil (15) começou em junho. Glosa no mês = dos lotes pagos naquele mês; no Dados (setembro), o acumulado do ano.\n")
