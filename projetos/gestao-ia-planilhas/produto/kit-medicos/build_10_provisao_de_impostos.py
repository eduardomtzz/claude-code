#!/usr/bin/env python3
"""Planilha 10 do Kit de Gestão para Médicos: Provisão de impostos, 13º e férias. Gera 10-provisao-de-impostos.xlsx
Exemplo alimentado pelos totais mensais da planilha 09 (dados.TOTAIS). Alíquota única do kit: 11 % (efetiva, combinada com o contador)."""
from ssg import *
import dados
...
titulo(cfg,"Configurações","Células amarelas: você preenche. Alíquotas e regras são as que a clínica combinou com o contador.",merge_to="H")
...
cfg["A9"]="Adicional sobre férias"; cfg["B9"]=1/3
cfg["A10"]="Encargos sobre 13º e férias"; cfg["B10"]=dados.ENCARGOS
cfg["A11"]="Mês em que o 13º é pago"; cfg["B11"]="Dezembro"
cfg["A12"]="Mês previsto de férias da equipe"; cfg["B12"]="Janeiro"
...
cfg["D8"]="Percentual que a clínica separa de cada real recebido, combinado com o contador. PJ médica no Simples (anexo III ou V, conforme o fator r) ou no lucro presumido tem alíquotas efetivas diferentes; esta planilha não afirma qual é a sua. No exemplo, 11 % (a mesma das planilhas 05 a 09 e 18)."
cfg["D9"]="Padrão usual: um terço. Confirme com o contador."
cfg["D10"]="Encargos sobre a folha (FGTS; INSS patronal se houver). No exemplo, 8 % (FGTS) sobre o 13º e as férias da recepcionista. O contador informa o percentual certo."
...
cfg.row_dimensions[8].height=70; cfg.row_dimensions[10].height=44
...
cfg["A14"]="Equipe: 13º e férias a provisionar (até 8 pessoas)"; rotulo(cfg["A14"])
hdr(cfg,15,["Pessoa","Custo mensal","Provisionar 13º?","Provisionar férias?","13º por mês","Férias por mês"])
E0=16
for i in range(NP):
    r=E0+i
...
notas=["Sócios: marque Sim no 13º se quiserem guardar uma retirada extra de dezembro (decisão da clínica, não obrigação); os encargos não se aplicam ao pró-labore aqui. Recepcionista CLT: 13º e férias obrigatórios, com encargos.",
       "Médica parceira por repasse não entra: o repasse é pago por produção (planilha 11). O que é obrigatório provisionar para cada vínculo depende do contrato e da legislação: confirme com o contador."]
for i,t in enumerate(notas): cfg.cell(row=ET+3+i,column=1,value=t); nota(cfg.cell(row=ET+3+i,column=1))
k=0
for n_,papel,tipo,custo,_ in dados.PESSOAS:
    if tipo.startswith("Repasse"): continue
    soc=tipo=="Pró-labore"
    for c,v in zip((1,2,3,4),(n_,custo,"Sim","Não" if soc else "Sim")): cfg.cell(row=E0+k,column=c,value=v)
    k+=1
# encargos só na CLT: a fórmula de 13º dos sócios não deve ter encargos → linha dos sócios com encargo zero via observação? Mantém simples: encargos valem para quem tem férias = Sim (CLT).
for i in range(NP):
    r=E0+i
    cfg.cell(row=r,column=5,value=f'=IF(OR(A{r}="",C{r}<>"Sim"),0,B{r}/12*(1+IF(D{r}="Sim",$B$10,0)))')
widths(cfg,(40,16,18,22,16,16,3,12)); cfg.sheet_view.showGridLines=False
...
hdr(en,4,["Mês","Entradas recebidas","Impostos pagos","13º pago","Férias pagas","Total usado no mês"])
for i in range(12):
    r=5+i
...
en["A20"]="Entradas recebidas = Entrou no mês (Painel da 09) menos \"Outras entradas\" (no exemplo, a devolução de despesa pessoal de abril, R$ 620, que não é receita e não paga imposto)."; nota(en["A20"])
widths(en,(14,18,16,14,16,18)); en.sheet_view.showGridLines=False
tot=dados.TOTAIS
for m in range(1,10):
...
notas=["Como ler: Provisão de impostos = entradas do mês × alíquota efetiva de Config. Compromisso do mês seguinte = imposto sobre as entradas deste mês (pago no mês que vem) + 13º e férias inteiros nos meses escolhidos em Config.",
       "Saldo provisionado é o que deveria estar numa conta separada da conta corrente da clínica. Compare com o extrato dessa conta todo fechamento de mês. Meses futuros mostram só 13º e férias até você lançar as entradas.",
...
como_usar(wb,"Provisão de impostos, 13º e férias",[
 ("O que esta planilha faz","Separa, todo mês, o dinheiro do imposto sobre o que entrou, do 13º e das férias da equipe. Mostra quanto deveria estar guardado, quanto já foi usado e avisa quando o que está separado não cobre o compromisso do mês seguinte."),
 ("Passo 1","Em Config, digite a alíquota efetiva que a clínica combinou com o contador (a planilha não afirma qual é a sua), o adicional de férias, os encargos, os meses de pagamento do 13º e das férias e a equipe com Sim / Não em cada provisão."),
 ...
 ("Com o contador","Leve esta aba para a reunião mensal: alíquota, provisões e o que foi pago. PJ médica: pergunte sobre o enquadramento (Simples anexo III ou V, fator r, presumido) e ajuste Config sempre que a orientação mudar."),
 ("Com a IA","Copie a tabela do Painel e use o prompt \"Caixa 04 · Preparar a reunião mensal com o contador\" ou \"Caixa 05 · Perguntas sobre provisão de impostos, 13º e férias\" da biblioteca do kit para montar a pauta e as perguntas."),
])
proteger(wb); salvar(wb,"10-provisao-de-impostos.xlsx","Provisão de impostos, 13º e férias · Kit de Gestão para Médicos")
