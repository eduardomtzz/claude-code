...
Leia também `../revisao-interna-1.md` (erros que os revisores pegaram nos kits 1 e 2) e `../../02-oferta/medicos.md`.

1. **Helpers**: `from ssg import *` (cores, F, fill, hdr, inp, calc, kpi, lista, widths, como_usar, proteger, salvar) e
   `import dados`. `dados.py` é a FONTE ÚNICA do exemplo: clínica, pessoas, custos, clientes, 38 casos (com cronograma de
   ...
   (provisao_10) e metas (metas_19). Nunca invente outro clínica, outros sócios, outros clientes ou outros números:
...
7. **Textos**: português do Brasil, direto, sem "você" acusatório. Nenhum conteúdo jurídico (peça, tese, prazo legal
   específico): o kit é gestão do clínica. Nunca prometer "nunca mais perder prazo": a planilha AVISA.
...
11. **Nomes de arquivo**: `NN-nome-em-minusculas-com-hifens.xlsx`, gerados por `build_NN_nome.py` na pasta `kit-medicos/`
    (salvar em `kit-medicos/NN-....xlsx`; `entrega/` é montada depois pelo empacotador).
