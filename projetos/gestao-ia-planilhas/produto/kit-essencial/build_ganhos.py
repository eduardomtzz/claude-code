#!/usr/bin/env python3
"""Planilha 3 do Kit IA no Trabalho · Essencial: Ganhos e Gastos. Gera 03-ganhos-e-gastos.xlsx"""
...
for c in ("A4","A5","A6","A7","A8","A9"): cfg[c].font=F(bold=True,color=UVA)
for c in ("B4","B5","B6","B9"): inp(cfg[c])
...
for i in range(24):
    src = f"$D${5+i}" if i<12 else f"$F${5+i-12}"
    cfg.cell(row=5+i,column=10,value=f'=IF({src}="","",{src})').font=F(size=9,color="B0A6C4")
dvs[1].formula1="=Config!$J$5:$J$28"
...
p["A8"]="Média de despesas dos últimos 3 meses"; p["C8"]=f'=IFERROR((SUMIFS({LE},{LB},"Despesa",{LA},">="&DATE({Y},{M}-2,1),{LA},"<="&DATE({Y},{M}+1,0)))/3,0)'
p["A9"]="Saldo acumulado equivale a"; p["C9"]='=IF(C8>0,G5/C8,0)'; p["D9"]="meses de despesa"
p["A10"]="Meta de reserva"; p["C10"]="=Config!$B$9"; p["D10"]="meses"
p["A11"]="Situação"; p["C11"]='=IF(C9>=C10,"Meta de reserva atingida",IF(C9>=C10/2,"No caminho: metade da meta","Reserva baixa: priorize guardar")) '
for r in (8,9,10,11): p.cell(row=r,column=1).font=F(color=TINTA,size=10); p.merge_cells(start_row=r,start_column=1,end_row=r,end_column=2)
p["C8"].number_format=BRL; p["C9"].number_format="0.0"; p["C10"].number_format="0"
for c in ("C8","C9","C10","C11"): p[c].font=F(bold=True,color=UVA,size=10)
p.merge_cells("C11:F11"); p["D9"].font=F(size=10,color=LILAS); p["D10"].font=F(size=10,color=LILAS)
# por categoria
p["A13"]="Para onde foi o dinheiro"; p["A13"].font=F(bold=True,size=13,color=UVA)
hdr(p,14,["Categoria de despesa","Valor no mês","% do total","Mês anterior","Barra"]); p.merge_cells("E14:H14")
for i in range(12):
    r=15+i; src=f"Config!$F${5+i}"
...
p.cell(row=27,column=1,value="Total"); p.cell(row=27,column=2,value="=SUM(B15:B26)"); p.cell(row=27,column=4,value="=SUM(D15:D26)")
for c in (1,2,4): p.cell(row=27,column=c).font=F(bold=True,color=UVA,size=10); p.cell(row=27,column=c).border=borda
p["B27"].number_format=BRL; p["D27"].number_format=BRL
p["A29"]="De onde veio o dinheiro"; p["A29"].font=F(bold=True,size=13,color=UVA)
hdr(p,30,["Categoria de receita","Valor no mês","% do total","Mês anterior"])
for i in range(12):
    r=31+i; src=f"Config!$D${5+i}"
    p.cell(row=r,column=1,value=f'=IF({src}="","",{src})'); calc(p.cell(row=r,column=1),center=False)
...
# ano
p["A44"]="O ano, mês a mês"; p["A44"].font=F(bold=True,size=13,color=UVA)
hdr(p,45,["Mês","Entrou","Saiu","Sobrou","Saldo ao fim do mês"])
for i in range(12):
    r=46+i
...
p.conditional_formatting.add("D46:D57", FormulaRule(formula=['D46<0'], font=Font(name="Arial",color="C8402E",size=10,bold=True)))
bc=BarChart(); bc.type="col"; bc.grouping="clustered"; bc.height=7.5; bc.width=18; bc.title=None; bc.style=2
bc.add_data(Reference(p,min_col=2,max_col=3,min_row=45,max_row=57),titles_from_data=True); bc.set_categories(Reference(p,min_col=1,min_row=46,max_row=57))
...
p.add_chart(bc,"G44")
p.cell(row=58,column=1,value="Meses sem lançamento aparecem zerados. Compare só os meses já fechados.").font=F(size=9,color=LILAS)
...
("Passo 1","Em Config, preencha o nome, o ano, o saldo que você tinha antes do primeiro lançamento e a meta de reserva (3 meses é um bom começo). Ajuste as categorias se quiser."),
...
("Passo 3","Em Painel, escolha o mês em Config e veja: entrou, saiu, sobrou, saldo, a pagar, reserva e as categorias com barra."),
...
wb.properties.creator="Seu Sócio Gestor"; wb.properties.title="Ganhos e Gastos · Kit IA no Trabalho Essencial"
