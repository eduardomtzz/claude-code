...
- 8 aulas em vídeo de 2 a 3 minutos (tela real + cartões + narração sintética + legenda). A oferta dizia "6 a 10 min";
  ajustado para "2 a 3 min" em todos os textos: aula curta e direta vende melhor do que aula esticada, e é o que existe.
...
| 2026-09-14 | **Pendente:** oferta do Kit de Gestão para Advogados em `02-oferta/advogados.md` (R$ 497 e 12×, order bump Essencial a R$ 27, escritório fictício "Ferraz & Lima Advocacia", sem modelo de contrato como bônus). Produção iniciada com essas premissas; mudar depois custa retrabalho só nos nomes e no bump. |

| 2026-09-14 | **Decidido (Eduardo):** narração provisória (Piper) fica em todos os vídeos até a conexão com o Google TTS; quando a chave `GOOGLE_TTS_API_KEY` existir, todas as narrações (aulas dos 3 kits, demos, vídeos de venda e de 15 s) são regravadas de uma vez. Os scripts de TTS já trocam de motor automaticamente pela variável de ambiente. |

| 2026-09-14 | **Decidido (Eduardo): iniciar o produto 4 (Kit de Gestão para Médicos) antes de os anteriores estarem no ar**, quebrando a regra "um produto só começa depois do anterior com ROAS medido" porque as contas externas ainda não existem. **Pendente:** oferta em `02-oferta/medicos.md` (R$ 697 e 12×, bump Essencial R$ 27, clínica fictícia "Clínica Vida Plena", sem contrato de parceria nem peça de divulgação como bônus). Produção iniciada com essas premissas. |

| 2026-09-24 | **Decidido (Eduardo):** mascote entra na comunicação visual do site (4 poses enviadas por ele). Uso registrado em `03-marca/guia.md`. Site volta a ter o caminho de compra simulado (`/checkout/<kit>/`, sem pedir dados, com botões que levam às páginas de aprovado, Pix e recusado) até os links da Kiwify existirem; quando o link real entra em `site/config.json`, o botão passa a apontar para ele e a página de simulação deixa de ser gerada. |
